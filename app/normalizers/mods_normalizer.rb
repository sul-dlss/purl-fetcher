...
class ModsNormalizer # rubocop:disable Metrics/ClassLength
  ...
  def normalize
    ...
    @ng_xml = Mods::OriginInfoNormalizer.normalize(mods_ng_xml: ng_xml)
    @ng_xml = Mods::SubjectNormalizer.normalize(mods_ng_xml: ng_xml)
    @ng_xml = Mods::NameNormalizer.normalize(mods_ng_xml: ng_xml)
    ...
    @ng_xml = Mods::TitleNormalizer.normalize(mods_ng_xml: ng_xml, label: label)
    @ng_xml = Mods::GeoExtensionNormalizer.normalize(mods_ng_xml: ng_xml, druid: druid)
  ...
  def normalize_notes
    ng_xml.xpath('//mods:note', mods: MODS_NS).each do |note_node|
      if ToMods::Note.note_type_to_abstract_type.include?(note_node['type']&.downcase) ||
         ToMods::Note.display_label_to_abstract_type.include?(note_node['displayLabel'])
        note_node.delete('type') unless note_node['type']&.downcase == 'summary'
        note_node.name = 'abstract'
      end
      if ToMods::Note.display_label_to_abstract_type.include? note_node['displayLabel']
  ...
  def normalize_abstracts
    ng_xml.xpath('/mods:mods/mods:abstract', mods: MODS_NS).each do |abstract_node|
      if ToMods::Note.note_type_to_abstract_type.include? abstract_node['type']&.downcase
        abstract_node['type'] =
          abstract_node['type'].downcase
      end
      if ToMods::Note.display_label_to_abstract_type.include? abstract_node['displayLabel']
  ...
  def normalize_usage_primary_for(base_node)
    ['genre', 'language', 'classification', 'subject', 'titleInfo', 'typeOfResource', 'name'].each do |node_name|

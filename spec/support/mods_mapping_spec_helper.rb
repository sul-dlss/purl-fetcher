  ...
  context 'when mapping to MODS (from cocina)' do
    ...
    it 'cocina Description maps to normalized MODS' do
      ...
      unless skip_normalization
        expect(actual_xml).to be_equivalent_to ModsNormalizer.normalize(mods_ng_xml: orig_mods_ng, druid: local_druid,
                                                                        label: label).to_xml
      end
    end
  end
  ...
  context 'when mapping from roundtrip MODS (to cocina)' do
    ...
    let(:roundtrip_mods_xml) do
      if defined?(roundtrip_mods)
        ModsNormalizer.normalize_purl_and_missing_title(mods_ng_xml: roundtrip_mods_ng, druid: local_druid,
                                                        label: label).to_xml
      end
    end
    ...
    let(:re_roundtrip_mods_xml) { ToMods::Description.transform(roundtrip_cocina_description, local_druid).to_xml }
    ...
    it 'roundtrip cocina maps to normalized roundtrip MODS' do
      ...
      #  originInfo nodes.
      if defined?(roundtrip_mods)
        normalized_rt_mods_xml = ModsNormalizer.normalize(mods_ng_xml: Nokogiri::XML(roundtrip_mods_xml), druid: local_druid,
                                                          label: label).to_xml
      end
...
RSpec.shared_examples 'cocina MODS mapping' do
  ...
  context 'when mapping from roundtrip cocina (to MODS)' do
    ...
    let(:roundtrip_mods_ng) { ToMods::Description.transform(roundtrip_cocina_description, local_druid) }
    ...
    it 'roundtrip cocina Description maps to expected MODS, normalized' do
      ...
      if defined?(roundtrip_cocina)
        normalized_mods_xml = ModsNormalizer.normalize(mods_ng_xml: Nokogiri::XML(mods_xml), druid: local_druid,
                                                       label: label).to_xml
      end
